...
import { getSupabase, getConfig } from "@inseme/cop-host";
...
export async function getNationalStats(consultationSlug) {
  ...
  const client = getNationalSupabase() || getSupabase();
...
export async function compareWithNational(consultationSlug, field) {
  ...
  const localCommune = getConfig("COMMUNITY_NAME", DEFAULT_COMMUNITY_NAME);
...
export function getShareText(consultation, options = {}) {
  ...
  const scopeLabel =
    {
      local: getConfig("COMMUNITY_NAME", DEFAULT_COMMUNITY_NAME),
      regional: getConfig("REGION_NAME", DEFAULT_REGION_NAME),
      national: "France",
    }[scope] || getConfig("COMMUNITY_NAME", DEFAULT_COMMUNITY_NAME);
  ...
  const hashtags = [
    getConfig("HASHTAG", DEFAULT_HASHTAG).replace("#", ""),
