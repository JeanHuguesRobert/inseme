...
import { GAME_PACKS, game_reducer } from "../lib/gameManager.js";
    ...
    >
      ...
      <div className="flex-1 w-full flex flex-col justify-center items-center gap-4">
        <div className="flex items-center gap-4">
          ...
          <button
            disabled={!canSpeak}
            onClick={() => onTip && onTip("normal")}
            onContextMenu={(e) => {
              e.preventDefault();
              onTip && onTip("royal");
            }}
            onDoubleClick={() => onTip && onTip("imperial")}
...
export default function BarmanDashboard() {
  ...
  const handleGuardSubmit = () => {
    if (!guardInput) return setGuardError("Sésame requis");
    if (guardInput === barSesame || guardInput === MASTER_SESAME) {
      if (typeof window !== "undefined") {
        localStorage.setItem(sesameStorageKey, "true");
      }
      setHasSesameSession(true);
  ...
  // Restore saved zone from localStorage
  React.useEffect(() => {
    const savedZone =
      typeof window !== "undefined" ? localStorage.getItem("inseme_barman_zone") : null;
    if (savedZone) {
