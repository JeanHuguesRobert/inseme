...
> **Gouverner l’autorité ; le marché peut fournir des bras.**  
...
Notre match n’est pas « l’agent le plus capable ». C’est :

```text
Qui peut encore décider
...
## 2. Bases doctrinales (ce qui ne se négocie pas)

### 2.1 La démocratie comme architecture d’autorité

La démocratie n’est pas un décor d’interface. C’est un ensemble de primitives :
...
```text
une personne + mille agents = une personne politiquement comptée
```
...
subject_kind      : living_person | demos | legal_entity | estate | other
community_kind    : cognitive | civic | institutional
visibility        : sovereign_private | restricted | public_open
hosting_mode      : self_hosted | nonprofit_hosted | commercial_hosted  # étude économique séparée
host_domain       : baronsmariani.org | lepp.fr | …   # multi-apex
```
...
## 3. Position relative à OpenClaw et aux runtimes du marché

### 3.1 Points communs (utiles)

- orchestrer le travail d’agents ;
- local-first / self-host possible ;
- open source et extensibilité ;
- présence multi-surface ;
- BYO models.
...
**Piste retenue comme pièce d’architecture, non comme identité du projet :**

```text
Mandant (vivant)
  → Mandate (borné, versionné, révocable)
    → AgentInstance (rôle, budget, log)
      → ExecutionRuntime ∈ { openclaw, nanobot, cop-tool, cli, humain, … }
...
## 4. Invariants anti-capture (« match non achetable »)

Ces invariants sont des exigences de **design**, pas des souhaits éditoriaux.
...
## 5. Architecture cible (rappel opérationnel)

Aligné avec [inseme#17](https://github.com/JeanHuguesRobert/inseme/issues/17) :

```text
DeploymentInstance  = tenant déployé (DB, domaine, config)
TwinRoot            = racine logique du sujet vivant
TwinInstance        = manifestation située dans un déploiement
AgentProfile        = capacité potentielle
AgentInstance       = acteur cognitif identifiable
Mandate             = autorisation bornée
AgentExecution      = invocation runtime (OpenClaw, tool, humain…)
```

Topologie dogfooding :

```text
Hub technique Inseme
├── DeploymentInstance civique (ex. Corte / LePP)
└── DeploymentInstance JHN
    └── TwinRoot JHN
        ├── Agent coordinator
        ├── Agent cartographer
        ├── Agent contradictor
        └── Agent coder
            └── (optionnel) ExecutionRuntime sous mandat
```

DNS cible (politique de marque) :

```text
jhn.baronsmariani.org     → instance personnelle JHN
*.lepp.fr                 → instances de redevabilité de mandat
même site Netlify (inseme) + un Supabase par instance + registry multi-apex
```
...
## 9. Formules de référence

> **Nous gouvernons l’autorité ; le marché peut fournir des bras.**

> **Pluraliser la capacité, non dissoudre la responsabilité.**

> **Augmenter la capacité sans ouvrir un marché parallèle de la souveraineté.**

> **LePP** : instances pour rendre un mandataire contrôlable.  
> **Instance personnelle** : communauté cognitive sous mandat, ancrée sur une personne vivante.  
> **Même infrastructure ; régimes distincts ; match non achetable.**
