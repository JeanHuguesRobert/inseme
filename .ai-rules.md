...
> See also [.rules.md](file:///c:/tweesic/inseme/.rules.md) at the root for general repository rules.
...
**ALWAYS** use the `Icon` component. **NEVER** import from `lucide-react` directly in UI components.
...
_Note: The `Icon` component is centrally managed in `@inseme/cop-host` and maps semantic names to Lucide icons._
...
## 11. Génère une interface web responsive (HTML/CSS ou React JSX) inspirée du style de Piet Mondrian. 
Exigences :

1. Grille stricte : uniquement lignes verticales et horizontales. Tous les blocs s’alignent sur cette grille.
2. Couleurs : rouge, bleu, jaune pour les éléments importants (CTA), blanc pour le fond, noir et gris clair pour les séparations.
3. Modularité : créer des blocs/cards/panels réutilisables et repositionnables.
4. Asymétrie équilibrée : blocs disposés de manière dynamique mais visuellement harmonieuse.
5. Minimalisme : pas de textures, ombres ou dégradés. Chaque élément a un rôle clair.
6. Typographie simple et lisible, hiérarchie claire.
7. Responsive : la grille et les blocs doivent s’adapter aux écrans desktop, tablette et mobile.
...
1. **Prefer Component-Local CSS**: Create `MyComponent.css` next to `MyComponent.jsx`.
2. **Local Import**: `import './MyComponent.css';` inside the JSX file.
3. **Use BEM-like naming**: (e.g., `.component-name__element--modifier`).
4. **Use ONLY semantic tokens**: No primitives.
5. **Include mobile-first responsive styles**.
...
- [ ] Component-specific CSS is imported locally (not in `index.css`)
